...
最近学习了一下同事老哥推荐的<a href="https://web.dev/learn/design" target="_blank">Learn Responsive Design!</a>，介绍如何做响应式设计。  
搭建这个博客的时候也考虑了适配不同size的屏幕以及手机，最开始就是凭着感觉和经验去适配，或者看看别的网站是怎么做的，  
Learn Responsive Design!这篇文章系统性地介绍了响应式设计各个方面，其中讲到的很多适配方法都让我有种"啊～原来还可以这么做"的感觉。  
所以把各种方法归纳总结一下，搞个demo～～
...
所以@media的语法应该是这样的(type是可选的，feature是必填的)：
...
打开你会发现它长得有点像浏览器，有后退、刷新等一些基础功能(minimal-ui)。这是通过在Web App Manifest文件中设置display属性来实现的。
可以通过display-mode来判断当前页面是以什么样的方式展示，进而设置不同的样式。
```css
...
如果改成vertical-rl，就变成垂直方向的文本流，从右至左堆叠。
...
max-height不能被替换为max-block-siz，一开始我还感觉很奇怪，但深想一下就明白了，媒体查询是用来判断设备的，宽就是宽，高就是高，都是真实存在的物理属性。
...
- 对于一些常见长单词的语言，例如德语，尽量不要设计很窄的列，还可以考虑使用css的hyphens属性(连字符)来自动添加连字符。
...
宏观布局通常不需要使用到媒体查询(Media Queries)，使用grid或者flex就可以自动达到你想要的效果(demo)。  
当然如果有一些特殊效果还是需要用到媒体查询，比如当屏幕较小时将一些内容收缩或者隐藏(首页右上&博客页导航栏)。
...
这时候我们就可以使用rem单位，rem是相对于根元素的字体大小的，所以我们可以通过改变根元素的字体大小来改变文本的大小。
...
使用这种方式会有一个非常明显的breakpoints，如果希望更加平滑的调整 可以使用vw单位，vw是相对于视口宽度的，所以可以通过改变视口宽度来改变文本的大小。
...
当然你可能不希望出现非常大或者非常小的情况，那么就需要用到clamp函数了。
...
同时单位记得不要写固定单位比如px，而可以用ch，ch是相对于字体大小的，这样就可以根据字体大小自动调整每行的字符数量了。
...
300w就是30px，srcset属性是对于src的完善，你还是需要提供src属性，只是浏览器会srcset中的图片来替换它。
...
根据浏览器视口的宽度来展示图片的不同宽度，比如在上述示例中，当浏览器视口宽度大于等于66em时，图片的宽度就不会超过33vw， 也就是视口的1/3，小于44em时，就占据视口的全部宽度。  
...
或者无法被阅读到的情况，提供alt属性对图片内容进行文字描述是必须的。
...
当然这样肯定是不够的，你得把各个设置颜色的地方都加在这里。这边有个比较好的实践，就是使用变量定义各种属性值。
```css
:root {
  --color-bg: #ffffff;
...
对于link，我们通常会给一个不同的颜色，但是如果你设置的颜色无法被用户分辨，那么这个颜色就没有意义了。所以不要只设置颜色，再进行加粗或者加上下划线等。  
对于文字和背景色，要考虑两者的对比度是否足够，最好显示设置background-color，避免使用浏览器默认。
...
对于输入框，加上type属性，比如email，number，tel等。这样在手机上就会调用对应的软键盘。加上autocomplete属性，比如name、email等， 可以实现自动填充。
