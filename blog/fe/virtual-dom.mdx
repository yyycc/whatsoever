...
```
所以说白了虚拟DOM就是一个JavaScript对象，而在运行的时候，渲染器会遍历整颗虚拟DOM树来构建真实的DOM树，这个过程被称之为挂载(mount)。
...
```
vue3在编译阶段做了很多事情，它会充分分析模板，提取关键信息负加在对应的虚拟节点上，从而在运行阶段，渲染器能够通过这些附加的信息提升性能。  
首先它对动静节点作了区分，还对静态节点做了提升。这样静态节点就只会被创建一次，在以后得每次渲染中都可以直接被重用，而不需要多次创建。
```js
...
使得动态节点的对比可以忽略DOM层级结构(需要使v-if和v-for节点也作为Block)。  
对于动态节点，还会额外打一个patchFlag标记，用于记录节点的动态内容。比如上面这个template的第二个p节点就会被打伤标记1，代表有动态的textContent。  
这样在运行阶段，这个节点的其他属性(如果有的话)就不需要在进行对比。
```js
