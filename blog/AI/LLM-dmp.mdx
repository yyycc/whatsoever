...
因为我们希望生成的文件可以直接运行，所以就需要将所有的import语句都写在模版代码中，并且在prompt中注明"保留模板中的import语句。"。  
...
## 各个模型比较
下面是针对gpt、claude以及gemini几个模型各自完成任务时，在价格、时长以及生成结果三个方面进行对比的结果。每个模型各自进行了10次测试。记录如下图：
