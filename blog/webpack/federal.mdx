...
module.exports = {
  plugins: [
    new ModuleFederationPlugin({
      name: 'main1',
      remotes: {
          'sub1': 'sub1@http://localhost:8081/remoteEntrySub1.js'
      },
      shared: ['lodash', 'md5']
    }),
  ],
...
```
它监听了afterPlugins钩子，这个钩子在WebpackOptionsApply的process方法中触发(可以看<a href="/blog/webpack-initial/#创建compiler" target="_black">webpack5 源码学习 - 初始化#创建Compiler</a>部分)。  
...
配置exposes就表示暴露自身模块，也就是remote，配置remotes就表示要消费远程模块，也就是host，至于share，它用来实现模块共享，我们放到最后将。
...
```
监听了make钩子，在这里做个简单的回忆，make钩子是webpack整个编译流程第二阶段也就是构建阶段的入口，
...
回到ContainerPlugin，这里的代码应该会有似曾相识的感觉，创建了ContainerEntryDependency，然后同样进入compilation的addEntry方法，后续的流程也是基本一致的。  
最后生成chunk的规则是每个expose生成一个chunk，所以像示例中的exposes，输出文件中就会多两个js文件。
```js:title=lib/container/ContainerPlugin.js
...
从而可以在后续factorizeModule阶段用获取到的\*\*moduleFactory创建相应的\*\*Module。
...
那么如何能有加载chunk的过程呢，就是通过调用import()。
所以你需要在入口文件中调用import('./bootstrap.js')，在js文件中去调用远程模块，比如要调用示例中的远程模块:
```js
import {sthfunction} from 'sub1/time'
```

### ContainerReferencePlugin
...

```js:title=lib/container/ContainerReferencePlugin.js
...
此外还有一个remote文件(remoteEntrySub1.js)，它会定义一个全局变量(如示例中，就是sub1)，去加载具体的模块(time、date)。  
...
接下来我们来看打包出来的js，看看host是如何使用远程模块的。看打包出来的文件最好在配置里加上下面这一段，不然打出来的代码可读性太低。
...
## 生成的文件
从生成的文件内容来分析一下，使用远程模块的功能是如何实现的。

### remote - remote.js
首先来看看这个最显眼的remote文件，很明显它的名字就是我们配置的filename。
```js:title=remoteEntrySub1.js
var sub1;
(() => {
  ...
  ...
  (() => {
    ...
    var moduleMap = {
      "./time": () => {
        return __webpack_require__.e(418).then(() => (() => ((__webpack_require__(418)))));
      },
      "./date": () => {
...
当前文件中只有一个j属性，__webpack_require__.f.j，它的作用就是去请求[chunkId].js，所以__webpack_require__.e(418)就是去请求418.js，
418.js的内容就是导出time模块。  
__webpack_require__(moduleId)的作用就是执行moduleId对应的模块(__webpack_modules__[moduleId])，返回module.exports。


### host - bundle.js
我们知道了remote的作用就是放一个全局变量，里面提供get函数来获取它暴露出的模块，那我们来看host是如何使用的。
```js:title=bundle.js
__webpack_require__.e(/* import() */ 468).then(__webpack_require__.bind(__webpack_require__, 468))
```
首先执行__webpack_require__.e，传入的chunkId是468，上面我们讲过host会生成两个chunk，一个主chunk，还有一个通过import()引入模块的chunk，
这个468就是后者。  
而在bundle.js中__webpack_require__.f的key除了j还有remotes，j的功能就是去下载468.js，我们来看remotes。
...
...
```
这边data是个数组，值为['default', './time', '584']，这部分的逻辑比较简单，就不赘述了。  
针对data的三个值执行了三次handleFunction:
第一次执行__webpack_require__('584', 0)。它是去执行moduleId对应的模块(__webpack_modules__[moduleId])，这个__webpack_modules__['584']在bundle的js中定义了:
```js:title=bundle.js
...
```
这里就是在请求remoteEntrySub1.js，这个module.exports最后就等于sub1。  
继续看handleFunction，__webpack_require__('584', 0).then的返回是作为next的第一个参数，
接下来执行onExternal，它的第一个入参external就是sub1。  
第二次执行__webpack_require__.I('default', 0)，它与share相关，我们先放一放。  
第三次执行sub1.get('./time', getScope).then的返回就是time模块执行后的返回，这样我们就拿到了time模块。

## share
最后来说一下share，share的作用是共享模块。也就是当本地项目和远程项目都需要用到同一个模块时，我们利用share配置实现共享，
如果版本一致，那么就不会加载远程的模块，而是直接使用本地的模块。  
如果不实用share，那么模块代码既会出现在本地的加载的js中，又会出现在请求的远程js中，也就是相同代码会加载两遍。

### SharePlugin
share功能通过consume和provide两个部分来实现。
```js:lib/sharing/Share.plugin.js
apply(compiler) {
...
```js:title=lib/sharing/ProvideSharePlugin.js
...
```
在全局变量sub1中，除了get方法，还有一个init。
__webpack_require__.S中会存储share模块，存储的时机就是第二次执行handleFunction(上面我们跳过的那一步)，也就是__webpack_require__.I('default', 0)。
__webpack_require__.I('default', 0)的作用就是去加载对应的js，并且调用initExternal去执行init方法，把模块传进去。  
init方法中也执行了__webpack_require__.I(name, initScope)，它在加载对应的js前会判断__webpack_require__.S是否已经存在了相同名称相同版本的模块，
如果存在，就不会再去下载js。这就实现了模块共享的效果。  
