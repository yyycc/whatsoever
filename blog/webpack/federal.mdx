...
module.exports = {
  plugins: [
    new ModuleFederationPlugin({
      name: 'main',
      remotes: {
          'sub': 'sub1@http://localhost:8081/remoteEntrySub.js'
      },
      shared: ['lodash', 'md5']
    }),
  ],
...
这就导致了md5的代码被加载了两次，因为在host项目中，我是会加载bundle.js和29.js的。  

接下来我加上share配置，host的bundle.js中没有md5的代码，remote的29.js中也没有md5的代码。而是各自多打包除了一个chunk，这个chunk就是md5的代码。  
这是host项目中对于md5部分的代码，只会加载这个chunk，也就是只会加载一次。
...
如果存在，就不会再去下载远程的js。这就实现了模块共享的效果。  
