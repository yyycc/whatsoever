...
## Before
上文<a target="_blank" href="/blog/webpack-cli">webpack5源码之旅 - webpack-cli</a>讲到任何方式处理完参数后，都会执行webpack(config)。  
接下来我们就进入webpack一探究竟。
...
可以返回上去看一下mergeExports方法，它处理get的时候也会用memoize包一下，所以webpack中的所有方法都会在第一次调用后被缓存起来。  
...
普通的模块会生成NormalModule，往上翻到Compilation部分会看到创建Compilation对象的时候传入的params中有一个normalModuleFactory，
这个factory就是创建NormalModule的地方。
NormalModule中会调用parse将源码解析为AST(抽象语法树)，从而解析出所有依赖。  
构建模块时，还会生成moduleGraph来记录模块之间引用信息。

### Chunk
输出时，webpack会将之前生成的所有module，按照一定的规则打成一个或者多个chunk。
...
其实webpack的各种能力基本都是通过Plugin实现的，在初始化Compiler后，会去挂载你配置的plugin，
...
理解Tapable是读webpack源码的基础，参考部分有相关文章，可以进行阅读学习，这边总结一下Tapable的几种钩子以及它们对应的注册和调用方法。
